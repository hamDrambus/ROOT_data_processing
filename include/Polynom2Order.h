...
protected:
	...
	std::vector<double> *_ys_last; //thees are supposed to stay valid poInt_ters and vectors before 'this' destruction
	...
	double _x0_in; //the poInt_t in which x is set to be 0, this is for convinience and in order to avoid too large coefficients
...
public:
	...
	virtual void operator ()(std::vector<double> &xs_in, std::vector<double> &ys_in, Int_t offset, Int_t N_poInt_ts, TVectorD &pars_out, double in_x0 = 0); //TODO: make const vector references
	...
	void FindIntersection(std::vector<double>::iterator &x_Int_ter, std::vector<double>::iterator &x_Int_ter2, double &x_Int_ter_exact, double &x_Int_ter_exact2,
