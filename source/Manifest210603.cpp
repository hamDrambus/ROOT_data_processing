...
namespace ParameterPile
{
	...
	{
		std::cout << "Initializing TEST 210603 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		...
		default_exp_manifest.in_folder = "../Data/210603/";
		default_exp_manifest.out_folder = "../Data/210603/results_vt/";
		...
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init210603_tests:: No event selection - processing everything" << std::endl;
		...
		default_exp_manifest.out_gnuplot_folder = "../Data/210603/results_vt/gnuplot/";
		...
		//default_exp_manifest.runs.push(32, 61); //16kV
		//default_exp_manifest.runs.push(63, 102); //12kV
		//default_exp_manifest.runs.push(104, 133); //20kV
		//default_exp_manifest.runs.push(135, 164); //16kV
		//default_exp_manifest.runs.push(166, 205); //12kV
		//default_exp_manifest.runs.push(207, 246); //12kV
		//default_exp_manifest.runs.push(248, 277); //16kV
		...
		//ALL PARAMETERS (THRESHOLDS) ARE THE SAME FOR ALL FOLDERS FOR 210603 DATA!
		experiment_manifest new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210603_Cd_20kV_850V_46V_12dB_0") + "/");
		...
		new_manifest.append_folder((new_manifest.name = "210603_Cd_20kV_850V_46V_12dB_1") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210603_Cd_16kV_850V_46V_12dB_0") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210603_Cd_16kV_850V_46V_12dB_1") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210603_Cd_16kV_850V_46V_12dB_2") + "/");
		manifest.manifests.push_back(new_manifest);

		default_exp_manifest.channels.info(1)->baseline.do_find_curved = false;

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210603_Cd_12kV_850V_46V_12dB_0") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210603_Cd_12kV_850V_46V_12dB_1") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210603_Cd_12kV_850V_46V_12dB_2") + "/");
		manifest.manifests.push_back(new_manifest);

#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded TEST 210603 manifests" << std::endl;
		return true;
	}
	...
	{
		std::cout << "Initializing 210603 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		area_vector SiPM_channels;
		SiPM_channels.push(32, 42);
		SiPM_channels.push(44, 44);
		...
		default_exp_manifest.data_voltage_of_zero_channel = -1.0;
		default_exp_manifest.in_folder = "../Data/210603/";
		default_exp_manifest.out_folder = "../Data/210603/results_v1/";
		default_exp_manifest.write_event_indices = false;
		default_exp_manifest.accepted_events_fname = "";
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init210603:: No event selection - processing everything" << std::endl;
			default_exp_manifest.accepted_events_data.clear();
		}
		default_exp_manifest.subruns_per_file = 1000;
		default_exp_manifest.runs_to_draw.push(0, 9999); //DRAW all
...
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded 210603 manifests" << std::endl;
		return true;
	}
};
