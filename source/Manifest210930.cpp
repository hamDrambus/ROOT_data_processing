...
namespace ParameterPile
{
	...
	{
		std::cout << "Initializing TEST 210930 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		...
		default_exp_manifest.in_folder = "../Data/210930/";
		default_exp_manifest.out_folder = "../Data/210930/results_vt/";
		...
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init210930_tests:: No event selection - processing everything" << std::endl;
		...
		default_exp_manifest.out_gnuplot_folder = "../Data/210930/results_vt/";
		...
		//default_exp_manifest.runs.push(1, 30); //f1 20.0kV, 2650V on THGEM1
		//default_exp_manifest.runs.push(32, 61); //f2 20.0kV, 2650V on THGEM1
		//default_exp_manifest.runs.push(63, 93); //f3 18.8kV, 2650V on THGEM1
		//default_exp_manifest.runs.push(95, 127); //f4 17.0kV, 2650V on THGEM1
		//default_exp_manifest.runs.push(129, 158); //f5 15.3kV, 2650V on THGEM1
		//default_exp_manifest.runs.push(160, 189); //f6 14.4kV, 2650V on THGEM1
		//default_exp_manifest.runs.push(191, 220); //f7 13.6kV, 2650V on THGEM1
		//default_exp_manifest.runs.push(223, 262); //f8 12.7kV, 2650V on THGEM1
		...
		*/
		...
		new_manifest.append_folder((new_manifest.name = "210930_Pu_20.0kV_800V_46V_12dB_2650V") + "/");
		...
		new_manifest.append_folder((new_manifest.name = "210930_Pu_20.0kV_800V_46V_12dB_2650V_1") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210930_Pu_18.8kV_800V_46V_12dB_2650V") + "/");
		manifest.manifests.push_back(new_manifest);

#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded TEST 210930 manifests" << std::endl;
		return true;
	}
	...
	{
		std::cout << "Initializing 210930 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		area_vector SiPM_channels;
		SiPM_channels.push(32, 42);
		SiPM_channels.push(44, 44);
		...
		default_exp_manifest.data_voltage_of_zero_channel = -1.0;
		default_exp_manifest.in_folder = "../Data/210930/";
		default_exp_manifest.out_folder = "../Data/210930/results_v2/";
		default_exp_manifest.write_event_indices = false;
		default_exp_manifest.accepted_events_fname = "../Post_processing/210930/results_v5/Pu_46V_18.8kV_800V_2650V/forms_Pu_peak/evetns.txt";
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init210930:: No event selection - processing everything" << std::endl;
			default_exp_manifest.accepted_events_data.clear();
		}
		default_exp_manifest.subruns_per_file = 1000;
		default_exp_manifest.runs_to_draw.push(0, 9999); //DRAW all
		...
		//MODIFY ONLY THIS BLOCK AND DISPLAY-RELATED VALUES FOR CHANNELS
		default_exp_manifest.out_gnuplot_folder = "../Data/210930/results_v2/";
		default_exp_manifest.out_picture_folder = "";
		...
		*/
		...
		//ALL PARAMETERS (THRESHOLDS) ARE THE SAME FOR ALL FOLDERS FOR 210930 DATA!
		//default_exp_manifest.channels.info(3)->baseline.do_find_curved = false;
		experiment_manifest new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210930_Pu_20.0kV_800V_46V_12dB_2650V") + "/");
		//this is a place to tweak individual channel for specific fields
...
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded 210930 manifests" << std::endl;
		return true;
	}
};
