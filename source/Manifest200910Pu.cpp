...
namespace ParameterPile
{
	...
	{
		std::cout << "Initializing TEST 200910 Pu manifests..." << std::endl;
#define PAIR std::pair<double, double>
		...
		default_exp_manifest.in_folder = "../Data/200910/";
		default_exp_manifest.out_folder = "../Data/200910/results_vt/";
		...
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init200910Pu_tests:: No event selection - processing everything" << std::endl;
		...
		default_exp_manifest.out_gnuplot_folder = "../Data/200910/results_vt/temp_gnuplot/";
		...
		default_exp_manifest.runs.push(167, 167); //Use only when all invalid files are deleted from folders.
		//List of valid files (runs):
		//default_exp_manifest.runs.push(167, 207); //20kV threshold 100mV
		//default_exp_manifest.runs.push(209, 223); //20kV threshold 130mV
		//default_exp_manifest.runs.push(225, 268); //18kV
		//default_exp_manifest.runs.push(270, 290); //16kV
		//default_exp_manifest.runs.push(292, 312); //12kV
		//default_exp_manifest.runs.push(314, 334); //10kV
		...
		ch_manifest.baseline.curved_range = PAIR(60, 130);
		ch_manifest.baseline.curved_center = PAIR(67, 112);
		ch_manifest.baseline.curved_trim = PAIR(1, 1);
		ch_manifest.display.do_draw = chs_to_draw.contains(0);
		ch_manifest.peaks.threshold = 0.036;
		...
		new_manifest.append_folder((new_manifest.name = "Pu_20kV_850V_46V_12dB_th100mV") + "/");
		...
		new_manifest.append_folder((new_manifest.name = "Pu_20kV_850V_46V_12dB_th130mV") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "Pu_18kV_850V_46V_12dB_th90mV") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "Pu_16kV_850V_46V_12dB_th90mV") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "Pu_12kV_850V_46V_12dB_th86mV") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "Pu_10kV_850V_46V_12dB_th86mV") + "/");
		manifest.manifests.push_back(new_manifest);

#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded TEST 200910 Pu manifests" << std::endl;
		return true;
	}
	...
	{
		std::cout << "Initializing 200910 Pu manifests..." << std::endl;
#define PAIR std::pair<double, double>
		area_vector SiPM_channels;
		SiPM_channels.push(32, 44);
		SiPM_channels.push(48, 59);
		...
		default_exp_manifest.data_voltage_of_zero_channel = -1.0;
		default_exp_manifest.in_folder = "../Data/200910/";
		default_exp_manifest.out_folder = "../Data/200910/results_v1/";
		default_exp_manifest.write_event_indices = false;
		default_exp_manifest.accepted_events_fname = "";
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init200910Pu:: No event selection - processing everything" << std::endl;
			default_exp_manifest.accepted_events_data.clear();
		}
		default_exp_manifest.subruns_per_file = 1000;
		default_exp_manifest.runs_to_draw.push(0, 9999); //DRAW all
		...
		//MODIFY ONLY THIS BLOCK AND DISPLAY-RELATED VALUES FOR CHANNELS
		default_exp_manifest.out_gnuplot_folder = "../Data/200910/results_v1/temp_gnuplot/";
		default_exp_manifest.out_picture_folder = "";//Do not save pictures
...
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded 200910 Pu manifests" << std::endl;
		return true;
	}
};
