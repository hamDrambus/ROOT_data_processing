...
namespace ParameterPile
{
	...
	{
		std::cout << "Initializing TEST 180830 X-ray manifests..." << std::endl;
#define PAIR std::pair<double, double>
		...
		default_exp_manifest.in_folder = "../Data/180830/";
		default_exp_manifest.out_folder = "../Data/180830/results_vt/";
		...
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init180830Xray_tests:: No event selection - processing everything" << std::endl;
		...
		//default_exp_manifest.runs.push(23, 37); //0kV, 35mm collimator

		default_exp_manifest.runs.push(23, 23);
		...
		ch_manifest.filter.n_points = 30;
		...
		new_manifest.append_folder((new_manifest.name = "180830_Xray_0kV_600V_12bB_49V_1ph_35mm_coll") + "/");
...
#undef PAIR
		...
		std::cout << "Loaded TEST 180830 X-ray manifests" << std::endl;
		return true;
	}
	...
	{
		std::cout << "Initializing 180830 X-ray manifests..." << std::endl;
#define PAIR std::pair<double, double>
		area_vector SiPM_channels;
		SiPM_channels.push(32, 44);
		SiPM_channels.push(48, 59);
		...
		default_exp_manifest.data_voltage_of_zero_channel = -1.0;
		default_exp_manifest.in_folder = "../Data/180830/";
		default_exp_manifest.out_folder = "../Data/180830/results_v1/";
		default_exp_manifest.write_event_indices = false;
		default_exp_manifest.accepted_events_fname = "";
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init180830Xray:: No event selection - processing everything" << std::endl;
			default_exp_manifest.accepted_events_data.clear();
		}
		default_exp_manifest.subruns_per_file = 1000;
		default_exp_manifest.runs_to_draw.push(0, 9999); //DRAW all
		...
		default_exp_manifest.runs.push(23, 37);
...
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded 180830 X-ray manifests" << std::endl;
		return true;
	}
};
