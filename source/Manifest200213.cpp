...
namespace ParameterPile
{
	...
	{
		std::cout << "Initializing TEST 200213 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		...
		default_exp_manifest.in_folder = "../Data/200213/";
		default_exp_manifest.out_folder = "../Data/200213/results_vt/";
		...
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init200213_tests:: No event selection - processing everything" << std::endl;
		...
		default_exp_manifest.out_gnuplot_folder = "../Data/200213/results_vt/temp_gnuplot/";
		...
		default_exp_manifest.runs.push(166, 166); //Use only when all invalid files are deleted from folders.
		...
		ch_manifest.baseline.do_find_curved = false;
		ch_manifest.baseline.baseline_range = PAIR(0, 24);
		...
		new_manifest.append_folder((new_manifest.name = "200213_Pu_20kV_850V_46V_12dB_th100mV") + "/");
		...
		new_manifest.append_folder((new_manifest.name = "200213_Pu_20kV_850V_46V_12dB_th80mV") + "/");
		manifest.manifests.push_back(new_manifest);

#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded TEST 200213 manifests" << std::endl;
		return true;
	}
	...
	{
		std::cout << "Initializing 200213 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		area_vector SiPM_channels;
		SiPM_channels.push(32, 44);
		SiPM_channels.push(48, 59);
		...
		default_exp_manifest.data_voltage_of_zero_channel = -1.0;
		default_exp_manifest.in_folder = "../Data/200213/";
		default_exp_manifest.out_folder = "../Data/200213/results_v1/";
		default_exp_manifest.write_event_indices = false;
		default_exp_manifest.accepted_events_fname = "";
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init200213:: No event selection - processing everything" << std::endl;
			default_exp_manifest.accepted_events_data.clear();
		}
		default_exp_manifest.subruns_per_file = 1000;
		default_exp_manifest.runs_to_draw.push(0, 9999); //DRAW all
		...
		//MODIFY ONLY THIS BLOCK AND DISPLAY-RELATED VALUES FOR CHANNELS
		default_exp_manifest.out_gnuplot_folder = "../Data/200213/results_v1/temp_gnuplot/";
		default_exp_manifest.out_picture_folder = "";//Do not save pictures
...
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded 200213 manifests" << std::endl;
		return true;
	}
};
