...
namespace ParameterPile
{
	...
	{
		std::cout << "Initializing TEST 210902 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		...
		default_exp_manifest.in_folder = "../Data/210902/";
		default_exp_manifest.out_folder = "../Data/210902/results_vt/";
		...
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init210902_tests:: No event selection - processing everything" << std::endl;
		...
		default_exp_manifest.out_gnuplot_folder = "../Data/210902/results_vt/gnuplot/";
		...
		//default_exp_manifest.runs.push(280, 309); //16kV
		//default_exp_manifest.runs.push(244, 278); //14kV
		//default_exp_manifest.runs.push(206, 242); //13kV
		//default_exp_manifest.runs.push(165, 204); //12kV
		//default_exp_manifest.runs.push(124, 163); //11kV
		//default_exp_manifest.runs.push(83, 122); //10kV
		//default_exp_manifest.runs.push(42, 81); //9kV
		//default_exp_manifest.runs.push(1, 40); //8kV
		...
		*/
		...
		//ALL PARAMETERS (THRESHOLDS) ARE THE SAME FOR ALL FOLDERS FOR 210902 DATA!
		experiment_manifest new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210902_Pu_16kV_850V_46V_12dB") + "/");
		...
		new_manifest.append_folder((new_manifest.name = "210902_Pu_14kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210902_Pu_13kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210902_Pu_12kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		default_exp_manifest.channels.info(3)->baseline.do_find_curved = false;

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210902_Pu_11kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210902_Pu_10kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210902_Pu_9kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "210902_Pu_8kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded TEST 210902 manifests" << std::endl;
		return true;
	}
	...
	{
		std::cout << "Initializing 210902 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		area_vector SiPM_channels;
		SiPM_channels.push(32, 42);
		SiPM_channels.push(44, 44);
		...
		default_exp_manifest.data_voltage_of_zero_channel = -1.0;
		default_exp_manifest.in_folder = "../Data/210902/";
		default_exp_manifest.out_folder = "../Data/210902/results_v1/";
		default_exp_manifest.write_event_indices = false;
		default_exp_manifest.accepted_events_fname = "";
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init210902:: No event selection - processing everything" << std::endl;
			default_exp_manifest.accepted_events_data.clear();
		}
		default_exp_manifest.subruns_per_file = 1000;
		default_exp_manifest.runs_to_draw.push(0, 9999); //DRAW all
...
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded 210902 manifests" << std::endl;
		return true;
	}
};
