...
namespace ParameterPile
{
	...
	{
		std::cout << "Initializing TEST 211021 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		...
		default_exp_manifest.in_folder = "../Data/211021/";
		default_exp_manifest.out_folder = "../Data/211021/results_vt/";
		...
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init211021_tests:: No event selection - processing everything" << std::endl;
		...
		default_exp_manifest.out_gnuplot_folder = "../Data/211021/results_vt/gnuplot/";
		...
		//default_exp_manifest.runs.push(139, 169); //f9 13.0kV
		//default_exp_manifest.runs.push(108, 137); //f8 11.7kV
		//default_exp_manifest.runs.push(71, 106); //f7 10.4kV
		//default_exp_manifest.runs.push(30, 69); //f6 8.4kV
		//default_exp_manifest.runs.push(1, 28); //f5 7.8kV _1 suffix
		//default_exp_manifest.runs.push(130, 143); //f4 7.8kV
		//default_exp_manifest.runs.push(89, 128); //f3 7.1kV
		//default_exp_manifest.runs.push(48, 87); //f2 5.8kV
		//default_exp_manifest.runs.push(1, 46); //f1 5.2kV
		...
		*/
		...
		//ALL PARAMETERS (THRESHOLDS) ARE THE SAME FOR ALL FOLDERS FOR 211021 DATA!
		//default_exp_manifest.channels.info(3)->baseline.do_find_curved = false;
		experiment_manifest new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_13.0kV_850V_46V_12dB") + "/");
		...
		new_manifest.append_folder((new_manifest.name = "211021_Pu_11.7kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_10.4kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_8.4kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_7.8kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_7.8kV_850V_46V_12dB_1") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_7.1kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_5.8kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);

		new_manifest = default_exp_manifest;
		new_manifest.append_folder((new_manifest.name = "211021_Pu_5.2kV_850V_46V_12dB") + "/");
		manifest.manifests.push_back(new_manifest);
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded TEST 211021 manifests" << std::endl;
		return true;
	}
	...
	{
		std::cout << "Initializing 211021 manifests..." << std::endl;
#define PAIR std::pair<double, double>
		area_vector SiPM_channels;
		SiPM_channels.push(32, 42);
		SiPM_channels.push(44, 44);
		...
		default_exp_manifest.data_voltage_of_zero_channel = -1.0;
		default_exp_manifest.in_folder = "../Data/211021/";
		default_exp_manifest.out_folder = "../Data/211021/results_v1/";
		default_exp_manifest.write_event_indices = false;
		default_exp_manifest.accepted_events_fname = "";
		if (!read_accepted_events(default_exp_manifest.accepted_events_fname, default_exp_manifest.accepted_events_data)) {
			std::cout << "Init211021:: No event selection - processing everything" << std::endl;
			default_exp_manifest.accepted_events_data.clear();
		}
		default_exp_manifest.subruns_per_file = 1000;
		default_exp_manifest.runs_to_draw.push(0, 9999); //DRAW all
...
#undef PAIR
		for (std::size_t m = 0, m_end_ = manifest.manifests.size(); m != m_end_; ++m)
			manifest.manifests[m].channels.sort();
		std::cout << "Loaded 211021 manifests" << std::endl;
		return true;
	}
};
